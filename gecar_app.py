...
# Apply filters
df_k_f = df_k[
    df_k["Wilayah"].isin(sel_wilayah_k) & df_k["Kelompok_Usia"].isin(sel_usia)
]
...
def insight(text, kind="info"):
    css = {"info": "insight-box", "warning": "insight-box-warning", "success": "insight-box-success"}
    st.markdown(f'<div class="{css.get(kind, "insight-box")}">💡 {text}</div>', unsafe_allow_html=True)
...
if page == "🏠 Ringkasan Eksekutif":
    ...
    with col1:
        ...
        fig = px.pie(
            wil_k, values="Jumlah", names="Wilayah",
        ...
        fig2 = px.bar(
            gu, x="Jenis_Kelamin", y="N", color="Kelompok_Usia",
    ...
    with col2:
        ...
        fig3 = px.bar(
            narsum_dist, x="Narsum", y="N", color="Wilayah",
            color_discrete_sequence=COLORS["wilayah"],
        ...
        fig4 = px.bar(
            dim_kii, x="N", y="Dimensi", orientation="h",
    ...
    with col_a:
        ...
        for _, row in dim_k_count.iterrows():
            st.markdown(f'<span class="dim-badge">{row["Dimensi"]}</span> &nbsp; **{row["Respons"]} respons**', unsafe_allow_html=True)
    with col_b:
    ...
    with col1:
        insight("Ketegangan & Konflik adalah dimensi paling banyak direspons dari kedua dataset — menunjukkan isu keamanan sebagai prioritas utama.", "warning")
    with col2:
        insight("Jayawijaya mendominasi jumlah respons KII (113 dari 167) dengan 3 narasumber berbeda, mencerminkan kompleksitas wilayah pegunungan.", "info")
    with col3:
        insight("Terdapat keselarasan antara masyarakat dan pemerintah soal Kebutuhan Utama dan Kelompok Rentan, namun gap pada Harapan & Kontribusi pemuda.", "success")
...
# ══════════════════════════════════════════════
    ...
    with tab1:
        ...
        with col1:
            ...
            fig = px.pie(wil, values="N", names="Wilayah", title="Distribusi per Wilayah",
                         color_discrete_sequence=COLORS["wilayah"], hole=0.35)
        ...
        with col2:
            ...
            fig = px.bar(gen, x="Jenis_Kelamin", y="N", title="Distribusi Jenis Kelamin",
                         color="Jenis_Kelamin",
        ...
        with col3:
            ...
            fig = px.pie(usia, values="N", names="Kelompok_Usia", title="Distribusi Kelompok Usia",
                         color_discrete_sequence=["#FF6D00", "#6200EA"], hole=0.35)
            fig.update_layout(height=320, margin=dict(t=40, b=10))
            st.plotly_chart(fig, use_container_width=True)
        ...
        fig = px.bar(wug, x="Wilayah", y="N", color="Jenis_Kelamin",
        ...
        q_count["Pertanyaan_Short"] = q_count["Pertanyaan"].apply(lambda x: x[:60] + "…" if len(x) > 60 else x)
        fig = px.bar(q_count.sort_values("N"), x="N", y="Pertanyaan_Short", orientation="h",
    ...
    with tab2:
        ...
        with col1:
            fig = px.bar(dim_k.sort_values("N"), x="N", y="Dimensi", orientation="h",
        ...
        with col2:
            fig = px.pie(dim_k, values="N", names="Dimensi",
        ...
        fig = px.bar(dw, x="Wilayah", y="N", color="Dimensi",
                     barmode="stack",
        ...
        fig = px.bar(du, x="Dimensi", y="N", color="Kelompok_Usia",
                     barmode="group",
        ...
        insight("Dimensi 'Ketegangan & Konflik' dan 'Kebutuhan Utama' mendominasi respons masyarakat, menunjukkan dua prioritas utama komunitas.", "warning")
        insight("Kelompok Anak cenderung lebih banyak berbicara tentang Harapan & Kontribusi, sementara Dewasa lebih banyak pada Ketegangan & Aktor.", "info")

    # ── Tab 3: Eksplorasi ──
    with tab3:
        st.markdown("### 🔍 Jelajahi Tanggapan per Dimensi")
        sel_dim = st.selectbox("Pilih Dimensi:", sorted(df_k_f["Dimensi"].unique()))
        sel_wil_exp = st.multiselect("Filter Wilayah:", df_k_f["Wilayah"].unique(), default=list(df_k_f["Wilayah"].unique()))

        filtered = df_k_f[
            (df_k_f["Dimensi"] == sel_dim) & (df_k_f["Wilayah"].isin(sel_wil_exp))
    ...
    with tab4:
        ...
        fig2 = px.imshow(ct2, text_auto=True, aspect="auto",
...
# ══════════════════════════════════════════════
    ...
    with tab1:
        ...
        with col1:
            ...
            fig = px.bar(n_dist, x="Narsum", y="N", title="Distribusi Narasumber",
                         color="Narsum", text="N",
        ...
        with col2:
            ...
            fig = px.pie(wil_kii, values="N", names="Wilayah",
                         title="Distribusi per Wilayah",
                         color_discrete_sequence=COLORS["wilayah"][:2], hole=0.4)
            fig.update_layout(height=350)
            st.plotly_chart(fig, use_container_width=True)

        # Narsum x Wilayah x Pertanyaan heatmap
        nw = df_kii_f.groupby(["Narsum", "Wilayah"]).size().reset_index(name="N")
        fig = px.bar(nw, x="Narsum", y="N", color="Wilayah",
                     barmode="stack",
        ...
        q_kii["Q_Short"] = q_kii["Pertanyaan"].apply(lambda x: x[:65] + "…" if len(x) > 65 else x)
        fig = px.bar(q_kii.sort_values("N"), x="N", y="Q_Short", orientation="h",
    ...
    with tab2:
        ...
        col1, col2 = st.columns([1, 1])
        with col1:
            fig = px.bar(dim_kii.sort_values("N"), x="N", y="Dimensi", orientation="h",
            ...
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = px.pie(dim_kii, values="N", names="Dimensi",
        ...
        ct_dn = pd.crosstab(dn["Dimensi"] if len(dn) > 0 else pd.Series([], name="Dimensi"),
                            dn["Narsum"] if len(dn) > 0 else pd.Series([], name="Narsum"),
                            values=dn["N"] if len(dn) > 0 else pd.Series([], name="N"),
                            aggfunc="sum").fillna(0)
        fig = px.imshow(ct_dn, text_auto=True, aspect="auto",
                        title="Heatmap: Dimensi × Narasumber KII",
                        color_continuous_scale="Reds")
        fig.update_layout(height=400)
        st.plotly_chart(fig, use_container_width=True)

        insight("Dimensi 'Dampak & Operasional LSM' adalah unik untuk KII — mencerminkan perspektif institusional yang tidak dimiliki masyarakat umum.", "info")
        insight("Narasumber GTY & JPY (Jayawijaya) paling banyak merespons isu Ketegangan & Konflik, konsisten dengan dinamika keamanan di Papua Pegunungan.", "warning")

    with tab3:
        st.markdown("### 🔍 Jelajahi Tanggapan KII per Dimensi")
        sel_dim_kii = st.selectbox("Pilih Dimensi KII:", sorted(df_kii_f["Dimensi"].unique()))
        sel_narsum = st.multiselect("Filter Narasumber:", df_kii_f["Narsum"].unique(), default=list(df_kii_f["Narsum"].unique()))

        filtered_kii = df_kii_f[
            (df_kii_f["Dimensi"] == sel_dim_kii) & (df_kii_f["Narsum"].isin(sel_narsum))
...
# ══════════════════════════════════════════════
    ...
    with col1:
        ...
        if len(dk_dim) > 0:
            ...
            fig = px.bar(w_dk, x="Wilayah", y="N", color="Wilayah",
                         color_discrete_sequence=COLORS["wilayah"],
                         text="N",
                         labels={"N": "Jumlah Respons"})
            fig.update_traces(textposition="outside")
            ...
            fig2 = px.bar(u_dk, x="Kelompok_Usia", y="N", color="Jenis_Kelamin",
                          color_discrete_map=COLORS["gender"],
                          barmode="group",
                          text="N",
    ...
    with col2:
        ...
        if len(dkii_dim) > 0:
            ...
            fig = px.bar(n_dkii, x="Narsum", y="N", color="Wilayah",
                         color_discrete_sequence=COLORS["wilayah"],
                         text="N",
                         barmode="stack",
                         labels={"N": "Jumlah Respons"})
            ...
            q_dkii["Q_Short"] = q_dkii["Pertanyaan"].apply(lambda x: x[:50] + "…" if len(x) > 50 else x)
            fig2 = px.pie(q_dkii, values="N", names="Q_Short",
                          color_discrete_sequence=px.colors.qualitative.Set2, hole=0.3)
            fig2.update_layout(height=280, margin=dict(t=10, b=10))
            st.plotly_chart(fig2, use_container_width=True)
        else:
...
# ══════════════════════════════════════════════
    ...
    fig2 = px.bar(
        comp_long, x="Dimensi", y="Persen", color="Kelompok",
        barmode="group",
    ...
    fig3 = px.bar(
        comp_df.sort_values("Gap (pp)"), x="Gap (pp)", y="Dimensi",
        orientation="h",
        color="Arah",
        color_discrete_map={
    ...
    col1, col2 = st.columns(2)
    with col1:
        insight("Masyarakat lebih banyak membahas Keamanan & Harapan — mencerminkan kebutuhan langsung dan aspirasi hidup sehari-hari.", "info")
    with col2:
        insight("KII lebih fokus pada Dampak Operasional LSM dan Rekomendasi — mencerminkan perspektif institusional dan program kerja.", "warning")
...
# ══════════════════════════════════════════════
    ...
    with col1:
        ...
        fig = px.bar(ug, x="Kelompok_Usia", y="N", color="Jenis_Kelamin",
                     barmode="group", color_discrete_map=COLORS["gender"],
                     text="N", labels={"N": "Jumlah Respons", "Kelompok_Usia": "Usia"})
        fig.update_traces(textposition="outside")
        fig.update_layout(height=300, margin=dict(t=10, b=10))
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig2 = px.bar(d_wil, x="N", y="Dimensi", orientation="h",
                      color="N", color_continuous_scale="Blues",
    ...
    with col2:
        ...
        if len(dkii_wil) > 0:
            ...
            fig = px.pie(n_wil, values="N", names="Narsum",
            ...
            fig2 = px.bar(dk_wil2, x="N", y="Dimensi", orientation="h",
                          color="N", color_continuous_scale="Reds",
                          labels={"N": "Respons", "Dimensi": ""})
            fig2.update_layout(height=380, coloraxis_showscale=False, margin=dict(t=10, b=10))
            st.plotly_chart(fig2, use_container_width=True)
    ...
    wilayah_insights = {
        "Jayawijaya": [
            ("warning", "Jayawijaya memiliki intensitas konflik tertinggi — dengan 3 narasumber KII yang konsisten menyebut ketegangan keamanan, penambangan rakyat di Yahukimo, dan ancaman OPM."),
            ("info", "Masyarakat Jayawijaya lebih banyak membahas Kebutuhan Utama (sandang-pangan-papan) akibat inflasi tertinggi nasional (5,65% - BPS 2024)."),
            ("success", "Terdapat inisiatif perdamaian melalui lembaga adat dan gereja sebagai pemersatu masyarakat di tengah dinamika konflik."),
        ],
        "Asmat, Papua Selatan": [
            ("info", "Asmat hanya terwakili dalam dataset Masyarakat — menunjukkan keterbatasan jangkauan KII di wilayah selatan Papua."),
            ("warning", "Isu dominan: akses layanan dasar (pendidikan, kesehatan) dan isolasi geografis yang memperparah ketergantungan masyarakat."),
            ("success", "Responden dewasa Asmat lebih banyak daripada anak — mencerminkan profil diskusi kelompok yang lebih matang."),
        ],
        "Sentani": [
            ("success", "Sentani relatif lebih stabil — dengan kehadiran Dinas Sosial dan Dinas Pendidikan sebagai narasumber KII yang mencerminkan aktifnya layanan pemerintah."),
            ("info", "Fokus KII Sentani lebih pada program pemberdayaan dan peace-building — berbeda dengan Jayawijaya yang lebih reaktif terhadap konflik."),
            ("warning", "Masyarakat Sentani tetap mencatat ketakutan terkait spillover konflik dari wilayah pegunungan."),
        ],
    }
